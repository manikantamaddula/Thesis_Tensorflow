...
with tf.device('/gpu:0'):
    def local_model(class1, class2):
        ...
        return tf.constant(W_temp.eval()), tf.constant(b_temp.eval()), tf.concat([class1input,class2input], 0), y, y_actual
    ...
    # W1=tf.constant(W1.eval())
    # b1=tf.constant(b1.eval())
    ...
    level2train_step = tf.train.RMSPropOptimizer(learning_rate).minimize(level2cross_entropy,var_list=[level2W,level2b])
    ...
    for i in range(1000000):
        ...
        summary, _ = sess.run([merged, level2train_step],
                              feed_dict={traininputlevel1: batch_xs, level2y_: batch_ys})
        # if i<=4000:
    ...
    class2testlabel = tf.concat([class2testlabel_1, class2testlabel_2], 1)
    # compare predicted label and actual label
    test_predicted=tf.nn.softmax(tf.matmul(tf.concat([class1test,class2test], 0), W1) + b1, name='test_predicted')
