...
with tf.device('/gpu:0'):
    ...
    # hidden layer2
    hidden2traininput = tf.nn.elu(tf.matmul(level2traininput, level2W) + level2b)
    hidden2W = tf.Variable(tf.truncated_normal([100, 100], stddev=0.1), name='W')
    hidden2b = tf.Variable(tf.random_normal([100]), name='b')

    # level3
    level3traininput = tf.nn.elu(tf.matmul(hidden2traininput, hidden2W) + hidden2b)
    ...
    k = 0.6
    learning_rate = tf.train.inverse_time_decay(starter_learning_rate, global_step, decay_rate=k,decay_steps=300)
    ...
    level3train_step = tf.train.RMSPropOptimizer(learning_rate).minimize(level3cross_entropy)
    ...
    level3testinput = tf.nn.relu(tf.matmul(hidden2testinput, hidden2W) + hidden2b)
