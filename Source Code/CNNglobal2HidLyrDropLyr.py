...
with tf.device('/gpu:0'):
    ...
    level2b = tf.Variable(tf.random_normal([100]), name='b')
    ...
    hidden2b = tf.Variable(tf.random_normal([100]), name='b')
    ...
    hidden3b = tf.Variable(tf.random_normal([100]), name='b')

    # hidden layer4
    hidden4traininput = tf.nn.relu(tf.matmul(hidden3traininput, hidden3W) + hidden3b)
    hidden4W = tf.Variable(tf.truncated_normal([100, 100], stddev=0.1), name='W')
    hidden4b = tf.Variable(tf.random_normal([100]), name='b')

    level3traininput = tf.nn.relu(tf.matmul(hidden4traininput, hidden4W) + hidden4b)
    ...
    level3b = tf.Variable(tf.random_normal([10]), name='b')
    ...
    traininputpredictions1 = sess.run(y_conv1, feed_dict={x1: localmodel_input, keep_prob1: 1.0})
    traininputpredictions2 = sess.run(y_conv2, feed_dict={x2: localmodel_input, keep_prob2: 1.0})
    traininputpredictions3 = sess.run(y_conv3, feed_dict={x3: localmodel_input, keep_prob3: 1.0})
    traininputpredictions4 = sess.run(y_conv4, feed_dict={x4: localmodel_input, keep_prob4: 1.0})
    traininputpredictions5 = sess.run(y_conv5, feed_dict={x5: localmodel_input, keep_prob5: 1.0})
    ...
    # validation input
    validationmodel_input, validationmodel_labels = mnist.validation.images, mnist.validation.labels
    validationinputpredictions1 = sess.run(y_conv1, feed_dict={x1: validationmodel_input, keep_prob1: 1.0})
    validationinputpredictions2 = sess.run(y_conv2, feed_dict={x2: validationmodel_input, keep_prob2: 1.0})
    validationinputpredictions3 = sess.run(y_conv3, feed_dict={x3: validationmodel_input, keep_prob3: 1.0})
    validationinputpredictions4 = sess.run(y_conv4, feed_dict={x4: validationmodel_input, keep_prob4: 1.0})
    validationinputpredictions5 = sess.run(y_conv5, feed_dict={x5: validationmodel_input, keep_prob5: 1.0})

    level2validationinput = tf.concat(
        [validationinputpredictions1, validationinputpredictions2, validationinputpredictions3,
         validationinputpredictions4,
         validationinputpredictions5], 1).eval()
    ...
    validationwriter = tf.summary.FileWriter('data/logs/validation')
    sess.run(tf.group(tf.initialize_all_variables(), tf.initialize_local_variables()))
    ...
    for i in range(100000):
        ...
        summary, _ = sess.run([merged, level3train_step],
                              feed_dict={level2traininput: batch_xs, level3y_: batch_ys, keep_prob: 0.5})
        trainwriter.add_summary(summary, i)

        #train accuracy
        # train_summary, Validation_predictions = sess.run([merged, level3y],
        #                                                       feed_dict={level2traininput: level2testinput,
        #                                                                  keep_prob: 1.0})
        # validationwriter.add_summary(validation_summary, i)

        # validation accuracy
        validation_summary,Validation_predictions = sess.run([merged,level3y], feed_dict={level2traininput: level2validationinput,level3y_: validationmodel_labels, keep_prob: 1.0})
        validationwriter.add_summary(validation_summary,i)

    end = int(round(time.time()*1000))
    print("Time for building global network: ")
    print(end - start)

    # Evaluate the whole model

    # test input
    testmodel_input, testmodel_labels = mnist.test.images, mnist.test.labels
    testinputpredictions1 = sess.run(y_conv1, feed_dict={x1: testmodel_input, keep_prob1: 1.0})
    testinputpredictions2 = sess.run(y_conv2, feed_dict={x2: testmodel_input, keep_prob2: 1.0})
    testinputpredictions3 = sess.run(y_conv3, feed_dict={x3: testmodel_input, keep_prob3: 1.0})
    testinputpredictions4 = sess.run(y_conv4, feed_dict={x4: testmodel_input, keep_prob4: 1.0})
    testinputpredictions5 = sess.run(y_conv5, feed_dict={x5: testmodel_input, keep_prob5: 1.0})

    level2testinput = tf.concat(
        [testinputpredictions1, testinputpredictions2, testinputpredictions3,
         testinputpredictions4,
         testinputpredictions5], 1).eval()
    # test_predicted = tf.nn.softmax(tf.matmul(h_fc1_drop, level3W) + level3b, name='test_predicted')
    test_predictions = sess.run(level3y, feed_dict={level2traininput: level2testinput, keep_prob: 1.0})
    prediction = tf.argmax(test_predictions, 1)
    correct_prediction = tf.equal(prediction, tf.argmax(testmodel_labels, 1))
    # correct_prediction = tf.equal(tf.argmax(test_predictions, 1), tf.argmax(tf.constant(mnist.test.labels), 1))
    ...
    # another approach for test accuracy
    testaccu=sess.run(evaluation_step,feed_dict={level2traininput: level2testinput,
                                                                     level3y_: testmodel_labels, keep_prob: 1.0})
    ...
    test_predictions = sess.run(level3y, feed_dict={level2traininput: level2traininput_temp, keep_prob: 1.0})

    correct_prediction = tf.equal(tf.argmax(test_predictions, 1), tf.argmax(tf.constant(mnist.train.labels), 1))
