...
with tf.device('/gpu:0'):
    ...
    h1input=tf.nn.relu(tf.matmul(level2traininput, level2W) + level2b)

    h1W = tf.Variable(tf.truncated_normal([10, 10],stddev=0.1),name='W')
    h1b = tf.Variable(tf.random_normal([10]),name='b')
    sess.run(tf.variables_initializer([h1W, h1b]))

    h2input = tf.nn.relu(tf.matmul(h1input, h1W) + h1b)

    h2W = tf.Variable(tf.truncated_normal([10, 10],stddev=0.1),name='W')
    h2b = tf.Variable(tf.random_normal([10]),name='b')
    sess.run(tf.variables_initializer([h2W, h2b]))

    level3traininput = tf.nn.relu(tf.matmul(h2input, h2W) + h2b)
    ...
    global_step = tf.Variable(0, trainable=False)
    boundaries = [30000, 100000,400000]
    values = [0.5, 0.01, 0.001,0.0001]
    learning_rate = tf.train.piecewise_constant(global_step, boundaries, values)


    tf.summary.scalar('learning_rate', learning_rate)
    # Passing global_step to minimize() will increment it at each step.
    level3train_step = (tf.train.GradientDescentOptimizer(learning_rate).minimize(level3cross_entropy, global_step = global_step))
    ...
    for i in range(1000000):
        ...
        summary, _ = sess.run([merged, level3train_step],
                              feed_dict={level2traininput: batch_xs, level3y_: batch_ys})
        # if i<=4000:
