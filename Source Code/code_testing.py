...
with tf.device('/gpu:0'):
    ...
    W1,b1=local_model(0,1,2,3,9)
    W2,b2=local_model(5,6,7,8,4)
    print(W1.eval())
    ...
    # hidden layer2
    hidden2traininput = tf.nn.tanh(tf.matmul(level2traininput, level2W) + level2b)
    ...
    level3traininput = tf.nn.tanh(tf.matmul(hidden2traininput, hidden2W) + hidden2b)
    ...
    starter_learning_rate = 0.01
    k = 0.5
    learning_rate = tf.train.inverse_time_decay(starter_learning_rate, global_step, decay_rate=k,decay_steps=1000)
    ...
    level3train_step = (tf.train.AdamOptimizer(learning_rate).minimize(level3cross_entropy, global_step= global_step))
